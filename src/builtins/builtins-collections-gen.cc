...
class WeakCollectionsBuiltinsAssembler : public CodeStubAssembler {
 ...
 protected:
  ...
  TNode<IntPtrT> FindEntry(TNode<Object> table, TNode<Object> key,
                           TNode<IntPtrT> first_probe_entry,
                           TNode<IntPtrT> entry_mask, Label* if_doesnt_exist);

  // Builds code that finds the ObjectHashTable entry for a {key} using the
  // comparison code generated by {key_compare}. The code returns with key index
...
TNode<IntPtrT> WeakCollectionsBuiltinsAssembler::FindEntry(
    TNode<Object> table, TNode<Object> key, TNode<IntPtrT> first_probe_entry,
    TNode<IntPtrT> entry_mask, Label* if_doesnt_exist) {
  ...
  return FindKeyIndex(table, first_probe_entry, entry_mask,
...
TF_BUILTIN(WeakMapLookupHashIndex, WeakCollectionsBuiltinsAssembler) {
  ...
  TNode<IntPtrT> existing_key_index =
      FindEntry(table, key, first_probe_entry, entry_mask, &if_doesnt_exist);

  Return(SmiTag(ValueIndexFromKeyIndex(existing_key_index)));
...
TF_BUILTIN(WeakCollectionSet, WeakCollectionsBuiltinsAssembler) {
  ...
  {
    // See HashTable::FirstProbe().
    var_first_probe_entry = WordAnd(var_hash, entry_mask);
    TNode<IntPtrT> existing_key_index = FindEntry(
        table, key, var_first_probe_entry, entry_mask, &if_doesnt_exist);
  ...
  BIND(&if_doesnt_exist);
  {
    ...
    auto is_not_live = [&](TNode<Object> entry_key, Label* if_same) {
      // This is the the negative form BaseShape::IsLive().
      GotoIf(Word32Or(IsTheHole(entry_key), IsUndefined(entry_key)), if_same);
    };

    // See HashTable::FindInsertionEntry().
    TNode<IntPtrT> insertion_key_index =
        FindKeyIndex(table, var_first_probe_entry, entry_mask, is_not_live);
