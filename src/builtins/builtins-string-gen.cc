 ...
 protected:
  ...
  void Generate(Variant variant, const char* method_name,
    ...
    // maybe_regexp is not a RegExp nor has [@@match / @@search] property.
    {
      ...
      TNode<Object> regexp =
          regexp_asm.RegExpCreate(context, native_context, initial_map,
                                  maybe_regexp, EmptyStringConstant());
      ...
      BIND(&slow_path);
      {
        TNode<Object> maybe_func = CAST(GetProperty(context, regexp, symbol));
        Callable call_callable = CodeFactory::Call(isolate());
