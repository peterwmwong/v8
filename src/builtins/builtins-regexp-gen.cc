...
TNode<Object> RegExpBuiltinsAssembler::RegExpCreate(
  ...
  return RegExpCreate(context, native_context, initial_map, maybe_string,
                      flags);
}

TNode<Object> RegExpBuiltinsAssembler::RegExpCreate(
    TNode<Context> context, TNode<Context> native_context,
    TNode<Map> initial_map, TNode<Object> maybe_string,
    TNode<String> flags) {
  TNode<String> const string = Select<String>(
      IsUndefined(maybe_string), [=] { return CAST(EmptyStringConstant()); },
      [=] { return ToString_Inline(context, maybe_string); },
      MachineRepresentation::kTagged);
  TNode<Object> regexp = CAST(AllocateJSObjectFromMap(initial_map));
  return CAST(CallRuntime(Runtime::kRegExpInitializeAndCompile, context, regexp,
                          string, flags));
}
...
// TODO(pwong): Update spec numbering/comments
...
TNode<Object> RegExpBuiltinsAssembler::MatchAllIterator(
    ...
    TNode<Object> maybe_string, char const* method_name) {
  ...
  Branch(IsRegExp(context, regexp), &if_regexp, &if_not_regexp);
  BIND(&if_regexp);
  {
    ...
    TNode<Object> species_constructor =
        CAST(SpeciesConstructor(native_context, regexp, regexp_fun));

    // b. Let flags be ? ToString(? Get(R, "flags")).
    TNode<Object> flags = CAST(
  ...
  BIND(&if_not_regexp);
  {
    ...
    var_matcher =
        RegExpCreate(context, native_context, regexp, StringConstant("g"));

    // c. If ? IsRegExp(matcher) is not true, throw a TypeError exception.
    GotoIfNot(IsRegExp(context, var_matcher), &throw_type_error);
    ...
    TNode<Object> last_index = CAST(LoadLastIndex(context, var_matcher, false));
    GotoIfNot(SmiEqual(SmiConstant(0), last_index), &throw_type_error);
    Goto(&create_iterator);
  }
  BIND(&throw_type_error);
  {
    CallRuntime(Runtime::kThrowIncompatibleMethodReceiver, context,
                StringConstant(method_name), regexp);
    Unreachable();
  }
  ...
  BIND(&create_iterator);
  {
    ...
    StoreObjectFieldNoWriteBarrier(
        iterator, JSRegExpStringIterator::kIteratingRegExpOffset, var_matcher);
    ...
    CSA_ASSERT(this, Word32Or(Word32Equal(var_global, zero),
                              Word32Equal(var_global, one)));
    CSA_ASSERT(this, Word32Or(Word32Equal(var_unicode, zero),
                              Word32Equal(var_unicode, one)));
#endif  // DEBUG

    // 7. Set iterator.[[Global]] to global.
    // 8. Set iterator.[[Unicode]] to fullUnicode.
    // 9. Set iterator.[[Done]] to false.
    TNode<Word32T> iterator_flags = Word32Or(
        Word32Or(Int32Constant(0),
                 Word32Shl(var_global,
                           Int32Constant(JSRegExpStringIterator::kGlobalBit))),
        Word32Shl(var_unicode,
                  Int32Constant(JSRegExpStringIterator::kUnicodeBit)));
    StoreObjectFieldNoWriteBarrier(
        iterator, JSRegExpStringIterator::kFlagsOffset, iterator_flags,
...
// ES#sec-regexp.prototype-@@matchAll
...
class RegExpStringIteratorAssembler : public RegExpBuiltinsAssembler {
 ...
 protected:
  TNode<Word32T> LoadFlags(TNode<HeapObject> regexp_string_iterator) {
    return LoadObjectField<Uint32T>(regexp_string_iterator,
                                    JSRegExpStringIterator::kFlagsOffset);
  }

  TNode<BoolT> HasDoneFlag(TNode<Word32T> flags) {
    return IsSetWord32(flags, JSRegExpStringIterator::kDoneMask);
  }

  TNode<BoolT> HasGlobalFlag(TNode<Word32T> flags) {
    return IsSetWord32(flags, JSRegExpStringIterator::kGlobalMask);
  }

  TNode<BoolT> HasUnicodeFlag(TNode<Word32T> flags) {
    return IsSetWord32(flags, JSRegExpStringIterator::kUnicodeMask);
  }

  void SetDoneFlag(TNode<Object> iterator, TNode<Word32T> flags) {
    TNode<Word32T> updated_flags =
        Word32Or(flags, Int32Constant(JSRegExpStringIterator::kDoneMask));
    StoreObjectFieldNoWriteBarrier(
        iterator, JSRegExpStringIterator::kFlagsOffset, updated_flags,
        MachineRepresentation::kWord32);
  }
};

// ES#%RegExpStringIteratorPrototype%.next
...
TF_BUILTIN(RegExpStringIteratorPrototypeNext, RegExpStringIteratorAssembler) {
  ...
  TNode<Word32T> flags = LoadFlags(receiver);
  ...
  TNode<Object> match = CAST(RegExpExec(context, regex, string));

  // 10. If match is null, then
  Branch(IsNull(match), &if_no_match, &if_match);
  BIND(&if_no_match);
  {
  ...
  BIND(&if_match);
  {
    ...
    BIND(&if_global);
    {
      ...
      TNode<Object> match_str = ToString_Inline(
          context,
          GetProperty(context, match, isolate()->factory()->zero_string()));
      ...
      // ii. If matchStr is the empty string,
      {
        ...
        TNode<Object> last_index = CAST(LoadLastIndex(context, regex, false));
        TNode<Object> this_index = CAST(ToLength_Inline(context, last_index));

        // 2. Let nextIndex be ! AdvanceStringIndex(S, thisIndex, fullUnicode).
        TNode<Object> next_index = CAST(AdvanceStringIndex(
            string, this_index, HasUnicodeFlag(flags), false));

        // 3. Perform ? Set(R, "lastIndex", nextIndex, true).
        StoreLastIndex(context, regex, next_index, false);
      ...
      Return(AllocateJSIteratorResult(context, match, FalseConstant()));
    }
    // b. Else,
    BIND(&if_not_global);
    {
