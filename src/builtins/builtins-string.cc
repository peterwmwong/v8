...
BUILTIN(StringRaw) {
  ...
  if (length > 0) {
    ...
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(
        isolate, result, Object::ToString(isolate, first_element));
    ...
    for (uint32_t i = 1, arg_i = 2; i < length; i++, arg_i++) {
      if (arg_i < argc) {
        ...
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(
            isolate, result,
            isolate->factory()->NewConsString(result, argument_string));
      }
      ...
      ASSIGN_RETURN_FAILURE_ON_EXCEPTION(
          isolate, result,
          isolate->factory()->NewConsString(result, element_string));
    }
