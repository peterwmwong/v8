...
void StringTrimAssembler::Generate(String::TrimMode mode) {
  ...
  // Check that {receiver} is coercible to Object and convert it to a String.
  Node* const string =
      ToThisString(context, receiver, String::kTrimModeMethod[mode]);
  ...
  if (mode == String::kTrimLeft || mode == String::kTrim) {
    ScanForNonWhiteSpaceOrLineTerminator(
        string_data, string_data_offset, is_stringonebyte, &var_start,
        string_length, IntPtrConstant(1), &return_emptystring);
  }
  if (mode == String::kTrimRight || mode == String::kTrim) {
    ScanForNonWhiteSpaceOrLineTerminator(
        string_data, string_data_offset, is_stringonebyte, &var_end,
        IntPtrConstant(-1), IntPtrConstant(-1), &return_emptystring);
  }
  ...
  BIND(&if_runtime);
  arguments.PopAndReturn(CallRuntime(Runtime::kStringTrim, context, receiver,
...
// TODO: Use Typed Nodes TNode (ex. TNode<Uint32T>)
void StringTrimAssembler::ScanForNonWhiteSpaceOrLineTerminator(
    ...
    Node* const increment, Label* const if_none_found) {
  Label if_stringisonebyte(this), out(this);

  auto for_each_index = [&](std::function<Node*(Node*)> get_char_code) {
    ...
    BIND(&loop);
    {
      ...
      GotoIfNotWhiteSpaceOrLineTerminator(
          UncheckedCast<Uint32T>(get_char_code(index)), &out);
      var_index->Bind(IntPtrAdd(index, increment));
      Goto(&loop);
    }
  };
  ...
  // Two Byte String
  for_each_index([&](Node* const index) {
    return Load(
