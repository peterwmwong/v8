...
class TypedArrayBuiltinsAssembler : public CodeStubAssembler {
 ...
 protected:
  ...
  void CallCCopyFastNumberJSArrayElementsToTypedArray(
      TNode<JSArray> source, TNode<JSTypedArray> dest,
...
void TypedArrayBuiltinsAssembler::SetTypedArraySource(
  ...
  // Check for possible range errors.
  {
    TNode<Smi> source_length =
        CAST(LoadObjectField(source, JSTypedArray::kLengthOffset));
    TNode<Smi> target_length =
        CAST(LoadObjectField(target, JSTypedArray::kLengthOffset));
    TNode<Smi> required_target_length = SmiAdd(source_length, SmiTag(offset));

    GotoIf(SmiGreaterThan(required_target_length, target_length),
           if_source_too_large);
  }
...
void TypedArrayBuiltinsAssembler::SetJSArraySource(
  ...
  BIND(&fast_c_call);
  CallCCopyFastNumberJSArrayElementsToTypedArray(source, target, source_length,
                                                 offset);
  Goto(&out);
  BIND(&out);
}
...
void TypedArrayBuiltinsAssembler::
    CallCCopyFastNumberJSArrayElementsToTypedArray(TNode<JSArray> source,
                                                   TNode<JSTypedArray> dest,
                                                   TNode<IntPtrT> source_length,
                                                   TNode<IntPtrT> offset) {
