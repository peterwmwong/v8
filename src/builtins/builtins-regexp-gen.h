...
class RegExpBuiltinsAssembler : public CodeStubAssembler {
 public:
  ...
  TNode<String> CallJSRegExpRegExpReplaceToSequentialString(
      TNode<String> subject, TNode<FixedArray> fixed_array,
      TNode<IntPtrT> length, TNode<String> sep, TNode<String> dest) {
    TNode<ExternalReference> func = ExternalConstant(
        ExternalReference::jsregexp_regexp_replace_to_sequential_string());
    ...
    return UncheckedCast<String>(
        CallCFunction(func,
                      ...
                      std::make_pair(MachineType::AnyTagged(), subject),
