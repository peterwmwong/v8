...
class WeakCollectionsBuiltinsAssembler : public CodeStubAssembler {
 ...
 protected:
  ...
  // Generates and sets the identity for a JS Receciver.
  TNode<Smi> CreateIdentityHash(TNode<Object> receiver);

  TNode<IntPtrT> EntryMask(TNode<IntPtrT> table_capacity) {
    return IntPtrSub(table_capacity, IntPtrConstant(1));
  }
  ...
  // {key} and returns the key index. If {key} cannot be found, jumps to
  // {if_doesnt_exist}. The key index is returned (instead of the entry index),
  // as it is assumed the calling code will need access the key and/or value.
  // It is faster to calculate the value index from the key index (see
  // ValueIndexFromKeyIndex()).
  TNode<IntPtrT> FindKeyIndexForKey(TNode<Object> table, TNode<Object> key,
                                    TNode<IntPtrT> key_hash,
                                    TNode<IntPtrT> entry_mask,
                                    Label* if_doesnt_exist);

  // Builds code that finds the ObjectHashTable entry with key that matches
  // {key} and calls {handle_key_found} with the found key index, key hash, and
  // table capacity. If the {key} does not have an identity hash OR cannot be
  // found, jumps to {if_doesnt_exist}.
  typedef std::function<void(TNode<IntPtrT> key_index, TNode<IntPtrT> key_hash,
                             TNode<IntPtrT> table_capacity)>
      HandleKeyFound;
  void FindKeyIndexForKey(TNode<Object> table, TNode<Object> key,
                          Label* if_doesnt_exist,
                          HandleKeyFound const& handle_key_found);
  ...
  TNode<Word32T> InsufficientCapacityToAdd(TNode<IntPtrT> table_capacity,
                                           TNode<IntPtrT> number_of_elements,
                                           TNode<IntPtrT> number_of_deleted);
  TNode<IntPtrT> KeyIndexFromEntry(TNode<IntPtrT> entry);
  TNode<IntPtrT> LoadNumberOfElements(TNode<Object> table);
  TNode<IntPtrT> LoadNumberOfDeleted(TNode<Object> table);
...
TNode<IntPtrT> WeakCollectionsBuiltinsAssembler::FindKeyIndexForKey(
    TNode<Object> table, TNode<Object> key, TNode<IntPtrT> key_hash,
    TNode<IntPtrT> entry_mask, Label* if_doesnt_exist) {
  ...
  return FindKeyIndex(table, key_hash, entry_mask, match_key_or_exit_on_empty);
}

void WeakCollectionsBuiltinsAssembler::FindKeyIndexForKey(
    TNode<Object> table, TNode<Object> key, Label* if_doesnt_exist,
    HandleKeyFound const& handle_key_found) {
  TNode<IntPtrT> table_capacity =
      SmiUntag(LoadFixedArrayElement(table, ObjectHashTable::kCapacityIndex));
  TNode<IntPtrT> key_hash = LoadJSReceiverIdentityHash(this, key);
  GotoIf(IntPtrEqual(key_hash, IntPtrConstant(PropertyArray::kNoHashSentinel)),
         if_doesnt_exist);

  TNode<IntPtrT> entry_mask = EntryMask(table_capacity);
  TNode<IntPtrT> key_index =
      FindKeyIndexForKey(table, key, key_hash, entry_mask, if_doesnt_exist);
  handle_key_found(key_index, key_hash, table_capacity);
}
...
TNode<IntPtrT> WeakCollectionsBuiltinsAssembler::LoadNumberOfDeleted(
    TNode<Object> table) {
  return SmiUntag(LoadFixedArrayElement(
...
TNode<Word32T> WeakCollectionsBuiltinsAssembler::InsufficientCapacityToAdd(
    TNode<IntPtrT> table_capacity, TNode<IntPtrT> number_of_elements,
    TNode<IntPtrT> number_of_deleted) {
...
TF_BUILTIN(WeakMapLookupHashIndex, WeakCollectionsBuiltinsAssembler) {
  ...
  Label if_key_not_found(this);

  auto if_key_found = [&](TNode<IntPtrT> key_index, TNode<IntPtrT> key_hash,
                          TNode<IntPtrT> capacity) {
    Return(SmiTag(ValueIndexFromKeyIndex(key_index)));
  };
  FindKeyIndexForKey(table, key, &if_key_not_found, if_key_found);

  BIND(&if_key_not_found);
  Return(SmiConstant(-1));
}
...
TF_BUILTIN(WeakCollectionSet, WeakCollectionsBuiltinsAssembler) {
  ...
  Label call_runtime(this), if_doesnt_exist(this), if_existing_hash(this),
      if_no_existing_hash(this);

  TNode<Object> table =
      LoadObjectField(CAST(collection), JSWeakCollection::kTableOffset);
  TNode<IntPtrT> table_capacity =
      SmiUntag(LoadFixedArrayElement(table, ObjectHashTable::kCapacityIndex));
  TNode<IntPtrT> entry_mask = EntryMask(table_capacity);
  ...
  BIND(&if_doesnt_exist);
  {
    ...
    GotoIf(Word32Or(ShouldRehash(number_of_elements, number_of_deleted),
                    InsufficientCapacityToAdd(
                        table_capacity, number_of_elements, number_of_deleted)),
           &call_runtime);
