...
(function HoleyStart() {
  ...
  assertTrue(%HasDictionaryElements(b));
  assertSame('a', b.join(''));
  assertSame('a' + ','.repeat(MIN_DICTIONARY_LENGTH - 1), b.join(','));
  assertSame(`a${','.repeat(MIN_DICTIONARY_LENGTH - 1)}`, b.join(','));
})();
...
(function HoleyEnd() {
  ...
  assertTrue(%HasDictionaryElements(b));
  assertSame('a', b.join(''));
  assertSame(`${','.repeat(MIN_DICTIONARY_LENGTH - 1)}a`, b.join(','));
})();
...
(function HoleyStartEnd() {
  ...
  assertSame(`${expectedStart}a${expectedEnd}`, b.join(','));
})();
...
(function HoleyMiddle() {
  ...
  assertTrue(%HasDictionaryElements(b));
  assertSame('ab', b.join(''));
  assertSame(`a${','.repeat(MIN_DICTIONARY_LENGTH - 1)}b`, b.join(','));
})();
...
(function HoleyStartMiddleEnd() {
  ...
  assertSame(`${expectedStart}a${expectedMiddle}b${expectedEnd}`, b.join(','));
})();
