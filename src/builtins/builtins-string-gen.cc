...
TF_BUILTIN(StringPrototypeMatchAll, StringBuiltinsAssembler) {
         ...
         &tostring_and_return_match_all_iterator);
  {
    ...
    auto if_regexp_call = [&] {
      // MaybeCallFunctionAtSymbol guarantees fast path is chosen only if object
      // is a string.
      CSA_ASSERT(this, IsString(receiver));
      var_receiver_string = UncheckedCast<String>(receiver);
    ...
    // TODO(pwong): Check the matchAll spec to see if we're doing too many
    // checks here.
